...
import com.infinities.keystonemiddleware.AuthProtocol.TokenEditor;
...
import com.infinities.keystonemiddleware.model.wrapper.AuthWrapper;
...
public class AuthProtocolTest {
	...
	public void setUp() throws Exception {
		...
		accessWrapper = JsonUtil.readJson(new File(authUrl.getPath()), AccessWrapper.class);
		URL revokedUrl = Thread.currentThread().getContextClassLoader().getResource("revokedwrapper.txt");
		revokedWrapper = JsonUtil.readJson(new File(revokedUrl.getPath()), RevokedToken.class);
	}
	...
	public void testCall() throws IOException {
		...
		context.checking(new Expectations() {
			...
			{
				exactly(1).of(client).target(identityUri);
				will(returnValue(target));
				exactly(1).of(target).path("/v2.0/tokens/revoked");
				will(returnValue(target));
				exactly(1).of(target).request(MediaType.APPLICATION_JSON_TYPE);
				will(returnValue(builder));
				exactly(1).of(builder).accept(MediaType.APPLICATION_JSON_TYPE);
				will(returnValue(builder));
				exactly(1).of(builder).header("X-Auth-Token", accessWrapper.getAccess().getToken().getId());
				will(returnValue(builder));
				exactly(1).of(builder).method("GET");
				will(returnValue(response));
				exactly(2).of(response).getStatus();
		...
		final String catalog = JsonUtil.toString(accessWrapper.getAccess().getServiceCatalog());
