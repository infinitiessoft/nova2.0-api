...
import com.infinities.keystonemiddleware.common.Cms;
import com.infinities.keystonemiddleware.model.Access;
import com.infinities.keystonemiddleware.model.AccessInfo;
import com.infinities.keystonemiddleware.model.Bind;
import com.infinities.keystonemiddleware.model.ServiceCatalogV2;
...
import com.infinities.keystonemiddleware.model.wrapper.RevokedWrapper;
...
public class AuthProtocol {
	...
	public AuthProtocol() throws MalformedURLException {
		...
		includeServiceCatalog =
				Config.Instance.getOpt(Config.Type.keystone_authtoken, "include_service_catalog").asBoolean();
		...
		int httpRequestMaxRetries =
				Config.Instance.getOpt(Config.Type.keystone_authtoken, "http_request_max_retries").asInteger();

		identityServer =
				new IdentityServer(includeServiceCatalog, Config.Instance.getOpt(Config.Type.keystone_authtoken,
						"identity_uri").asText(), Config.Instance.getOpt(Config.Type.keystone_authtoken, "auth_uri")
						.asText(), Config.Instance.getOpt(Config.Type.keystone_authtoken, "auth_host").asText(),
						Config.Instance.getOpt(Config.Type.keystone_authtoken, "auth_port").asInteger(), Config.Instance
								.getOpt(Config.Type.keystone_authtoken, "auth_protocol").asText(), Config.Instance.getOpt(
						...
						httpRequestMaxRetries, Config.Instance.getOpt(Config.Type.keystone_authtoken, "auth_version")
								.asText());
		...
		String memcacheSecurityStrategy =
				Config.Instance.getOpt(Config.Type.keystone_authtoken, "memcache_security_strategy").asText();

		tokenCache =
				new TokenCache(Config.Instance.getOpt(Config.Type.keystone_authtoken, "token_cache_time").asInteger(),
						Config.Instance.getOpt(Config.Type.keystone_authtoken, "hash_algorithms").asList(), Config.Instance
								.getOpt(Config.Type.keystone_authtoken, "cache").asText(), Config.Instance.getOpt(
		...
		tokenRevocationListCacheTimeout =
				Config.Instance.getOpt(Config.Type.keystone_authtoken, "revocation_cache_time").asInteger();
		checkRevocationsForCached =
				Config.Instance.getOpt(Config.Type.keystone_authtoken, "check_revocations_for_cached").asBoolean();
	...
	private Map<String, String> buildUserHeaders(TokenWrapper tokenInfo) throws IOException {
		...
		if (includeServiceCatalog && authRef.hasServiceCatalog()) {
			...
			rval.put("X-Service-Catalog", JsonUtil.toString(catalog));
		}

		return rval;
	}

	private List<Access.Service> v3ToV2Catalog(List<com.infinities.keystonemiddleware.model.v3.Token.Service> data) {
		List<Access.Service> v2Services = new ArrayList<Access.Service>();
		for (com.infinities.keystonemiddleware.model.v3.Token.Service v3Service : data) {
			...
			for (com.infinities.keystonemiddleware.model.v3.Token.Service.Endpoint v3Endpoint : v3Service.getEndpoints()) {
	...
	private TokenWrapper loads(String verified) throws IOException {
		try {
			TokenV3Wrapper v3Wrapper = JsonUtil.readJson(verified, TokenV3Wrapper.class);
			if (v3Wrapper.getToken() == null) {
				logger.debug("v2.0 token format: {}", verified);
				return JsonUtil.readJson(verified, AccessWrapper.class);
			} else {
				return v3Wrapper;
			}
		} catch (Exception e) {
	...
	private RevokedWrapper getTokenRevocationList() throws IOException, SslPemException {
		...
		if (listIsCurrent) {
			if (tokenRevocationListProp == null) {
				tokenRevocationListProp = JsonUtil.readJson(new File(revokedFileName), RevokedWrapper.class);
			}
		} else {
	...
	private void setTokenRevocationList(String value) throws IOException {
		...
		tokenRevocationListProp = JsonUtil.readJson(value, RevokedWrapper.class);
	...
	private void removeAuthHeaders(ContainerRequestContext env) {
		String[] authHeaders =
				new String[] { "X-Identity-Status", "X-Domain-Id", "X-Domain-Name", "X-Project-Id", "X-Project-Name",
						"X-Project-Domain-Id", "X-Project-Domain-Name", "X-User-Id", "X-User-Name", "X-User-Domain-Id",
						"X-User-Domain-Name", "X-Roles", "X-Service-Catalog", "X-User", "X-Tenant-Id", "X-Tenant-Name",
						"X-Tenant", "X-Role" };
